...
def open_enrico(source_dir, *, max_samples: Optional[int]):
    ...
    def iterate_samples():
        for idx, fname in enumerate(input_samples):
            ...
            # background image
            background_orig_path = str(fname).replace('semantic_annotations', 'LaMa_inpainted_background_images').replace('.json', '_mask001.png')
    ...
    def iterate_samples():
        for idx, fname in enumerate(input_samples):
            ...
            for element in elements:
                ...
                patch_masks.append(patch_mask)
            # background image
            background_orig_path = str(fname).replace('json_png_pseudo_label_blue_bbox_filtered', 'LaMa_3x_stringOnly_inpainted_background_images_edge').replace('.json', '_mask001.png')
            assert os.path.isfile(background_orig_path)
            background_orig = PIL.Image.open(background_orig_path)
    ...
    def iterate_samples():
        for idx, fname in enumerate(input_samples):
            ...
            for i, e in enumerate(_elements):
                ...
                for j, ee in enumerate(_elements):
                    if i != j:
                        ...
                        x2_min = min([x2, xx2])
                        y2_min = min([y2, yy2])
                        if float((x2_min - x1_max) * (y2_min - y1_max)) / float((x2 - x1) * (y2 - y1)) >= 0.95:
                            valid = False
                            break
                valid_list.append(valid)
            _elements = [e for i, e in enumerate(_elements) if valid_list[i]]
            ...
            for element in elements:
                ...
                patch_masks.append(patch_mask)
            # background image
            background_orig_path = str(fname).replace('manual_json_png_gt_label', 'manual_LaMa_stringOnly_inpainted_background_images').replace('.json', '_mask001.png')
            assert os.path.isfile(background_orig_path)
            background_orig = PIL.Image.open(background_orig_path)
            background_orig = background_orig.resize((1024, 1024), resample=PIL.Image.BILINEAR)
            background_orig = np.array(background_orig)
    ...
    def iterate_samples():
        for idx, fname in enumerate(input_samples):
            ...
            for element in elements:
                ...
                patch_masks.append(patch_mask)
            # background image
            background_orig_path = str(fname).replace('manual_json_png_gt_label', 'manual_LaMa_3x_stringOnly_inpainted_background_images').replace('.json', '_mask001.png')
            assert os.path.isfile(background_orig_path)
            background_orig = PIL.Image.open(background_orig_path)
            background_orig = background_orig.resize((1024, 1024), resample=PIL.Image.BILINEAR)
            background_orig = np.array(background_orig)
    ...
    def iterate_samples():
        for idx, fname in enumerate(input_samples):
            ...
            for element in elements:
                ...
                patch_masks.append(patch_mask)
            # background image
            background_orig_path = str(fname).replace('json_png_manual_label', 'LaMa_stringOnly_inpainted_background_images').replace('.json', '_mask001.png')
            assert os.path.isfile(background_orig_path)
            background_orig = PIL.Image.open(background_orig_path)
            background_orig = background_orig.resize((1024, 1024), resample=PIL.Image.BILINEAR)
            background_orig = np.array(background_orig)
