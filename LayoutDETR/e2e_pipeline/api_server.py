...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            if width > BannerConfig.MAX_IMG_WIDTH or height > BannerConfig.MAX_IMG_HEIGHT:
...
def content_generation():
    ...
    if not browser:
        ...
        for opt in RendererConfig.BROWSER_CONFIG:
            options.add_argument(opt)

        browser = Chrome(executable_path='/usr/bin/chromedriver', options=options)
        browser.set_window_size(BannerConfig.MAX_IMG_WIDTH, BannerConfig.MAX_IMG_HEIGHT)
    ...
    generated_screenshots, generated_htmls = generate_banners(loaded_models[model_id], background_image_path,
    ...
    for gen_screenshot, gen_html in zip(generated_screenshots, generated_htmls):
...
def update_files():
    ...
    w_page, h_page = BannerConfig.MAX_TIMG_WIDTH, BannerConfig.MAX_TIMG_HEIGHT  # thumbnail resolution
    global browser
    if not browser:
        # initialize Chrome based web driver for html screenshot
        options = webdriver.ChromeOptions()
    ...
    for item in edited_htmls:
        ...
        try:
            ...
            try:
                ...
                # To-Do: maintain an application log
                print(e)
                cur_status = "error"
