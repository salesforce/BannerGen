...
def consolidate_line(boxes, line_threshold=0.75):
    ...
    for idx in range(1, boxes_len):
        ...
        if y_dist > line_threshold:
            ...
            y_cnt, y_sum, y_avg, h_max = 1, boxes[idx][1], float(boxes[idx][1]), max(
                [boxes[idx][3] - boxes[idx][1] + 1, 1])
            line = [boxes[idx]]
        else:
...
def boxes_of_texts(original, original_for_draw, mdl_t):
    ...
    result = mdl_t.ocr('temp.png', det=True, rec=False, cls=False)
...
def boxes_of_faces(original, original_for_draw, mdl_f):
...
def boxes_of_saliencies_onetime(original, mdl_s):
    ...
    d1, _, _, _, _, _, _ = mdl_s(inputs_test)
    pred = d1[:, 0, :, :]
...
def auto_center(original, saliencies, target_width, target_height, text_prioritized, face_prioritized, mdl_s,
                mdl_f=None, mdl_t=None):

    height, width, _ = original.shape
    original_for_draw = Image.fromarray(cv2.cvtColor(original, cv2.COLOR_BGR2RGB))
    texts = faces = []
    if mdl_t:
        texts, original_for_draw = boxes_of_texts(original, original_for_draw, mdl_t)
    if mdl_f:
        faces, original_for_draw = boxes_of_faces(original, original_for_draw, mdl_f)

    if target_width/float(width) == target_height / float(height):
        saliency = []
        r = target_width/float(width)
        for sals in saliencies:
            saliency.append([int(s*r) for s in sals])
        print('faster')
    else:
        saliency, original_for_draw, _ = boxes_of_saliencies(original, original_for_draw, mdl_s)

    if len(texts) == 0 and len(faces) == 0 and len(saliency) == 0:
        ...
        if len(saliency) > 0:
            count += 1
            saliencies_center, saliencies_max = boxes_center_max(saliency)
...
def smart_crop(image, saliencies, target_width, target_height, do_resize, text_prioritized, face_prioritized,
               model_supres, model_saliency, model_text=None, model_face=None):
    # pil to cv: RGB to BGR
    original = np.array(image)[:, :, ::-1].copy()
    ...
    if do_resize:
        original = auto_resize(original, target_width, target_height, model_supres)
...
def main():
    ...
    smart_crop(args["image"], args["width"], args["height"], not args["no_resize"],
               args["text_prioritized"], args["face_prioritized"], mdl_supres, mdl_saliency)
"""
