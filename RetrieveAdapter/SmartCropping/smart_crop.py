...
def auto_resize(image, target_width, target_height, model):
    ...
    if not need_upsize:
        ...
        cell[:, 1] *= 2 / w
        #pdb.set_trace()
        image = batched_predict(model, ((image - 0.5) / 0.5).cuda().unsqueeze(0), coord.unsqueeze(0), cell.unsqueeze(0),
                                bsize=30000)[0]
        #pdb.set_trace()
...
def auto_center(original, target_width, target_height, text_prioritized, face_prioritized, mdl_s,
    ...
    saliencies, original_for_draw, _ = boxes_of_saliencies(original, original_for_draw, mdl_s)
...
def smart_crop(image, target_width, target_height, do_resize, text_prioritized, face_prioritized,
